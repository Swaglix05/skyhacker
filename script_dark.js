...
(function(){
  ...
  // there are two attempt hash elements in the markup: a short one in the brute area
  // and a labeled one in the dark-hash box. Update both when available.
  const attemptHash = document.getElementById('attempt-hash');
  const attemptHashInput = document.getElementById('attempt-hash-input');
  ...
  // Attach toggle handler (both light and dark scripts can use same button)
  if (toggleHackerBtn) toggleHackerBtn.addEventListener('click', () => {
    const isDark = document.body.classList.toggle('dark-mode');
    // show/hide darkSide handled by CSS, but ensure initial hash updates
    if (isDark) {
      const raw = (passwordInput && passwordInput.value) ? passwordInput.value : '';
      if (passwordInputDark) {
        passwordInputDark.value = (raw.split ? raw.split('').join(' ') + (raw.length ? ' ' : '') : raw);
      }
      if (passwordInput) computeHash(raw).then(h => { if (passwordHashEl) passwordHashEl.value = h; });
      // position hash box
  ...
  if (passwordInput) passwordInput.addEventListener('input', () => {
    ...
    computeHash(passwordInput.value || '').then(h => { if (passwordHashEl) passwordHashEl.value = h; });
  });

  // When the dark-mode password input is edited, keep the hidden main password in sync
  if (passwordInputDark) passwordInputDark.addEventListener('input', (ev) => {
    // remove spaces and pipe characters to form raw password
    const raw = (passwordInputDark.value || '').replace(/[\s|]/g, '');
    // update the hidden main password input
    if (passwordInput) passwordInput.value = raw;
    // rewrite the dark input to force a single space between characters for alignment
    const spaced = raw.split('').join(' ') + (raw.length ? ' ' : '');
    if (passwordInputDark.value !== spaced) passwordInputDark.value = spaced;
    // update computed hash display
    computeHash(raw).then(h => { if (passwordHashEl) passwordHashEl.value = h; });
  });

  // Reposition the hash box next to the password input (vertically centered)
  function positionHashBox() {
    if (!darkHashBox || !passwordInput) return;
  ...
  if (solveBtn) solveBtn.addEventListener('click', startBruteForce);
  if (stopBtn) stopBtn.addEventListener('click', () => { bruteCancel = true; });
  ...
  async function startBruteForce() {
    ...
    if (attemptHashInput) attemptHashInput.value = '';

    // reset attempts counter
    attemptsCount = 0;
    if (attemptsCountEl) attemptsCountEl.textContent = 'Solves since: 0';

    const target = (passwordInput && passwordInput.value) ? passwordInput.value : '';
    ...
    // English keyboard-like characters (letters + digits + common symbols)
    ...
    for (let len = 1; len <= maxLen && !bruteCancel && !found; len++) {
      ...
      while (!bruteCancel) {
        const attempt = indices.map(i => chars[i]).join('');
        ...
        attempts++;
        attemptsCount++;
        if (attemptsCountEl) attemptsCountEl.textContent = 'Solves since: ' + attemptsCount;
        ...
        while (pos >= 0) {
          indices[pos]++;
          if (indices[pos] >= chars.length) { indices[pos] = 0; pos--; continue; }
          break;
        }
    ...
    if (found) {
      if (attemptDisplay) attemptDisplay.value = 'FOUND: ' + attemptDisplay.value;
    } else if (bruteCancel) {
